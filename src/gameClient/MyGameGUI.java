...
public class MyGameGUI extends JFrame implements ActionListener, MouseListener, Runnable {
    ...
    private void initGame(int gameNumber) {
        ...
        try {
            ...
            for (int a = 0; a < rs; a++) {
                if (a < fs) {
                    game.addRobot(fruits.get(a).getEdge().getSrc());//need pos
                    nodeData temp = (nodeData) g.getNode(fruits.get(a).getEdge().getSrc());
                    temp.setRobot(true);
                } else {
    ...
    private void porpor() {
        ...
        xMin=xMin-epsilon;xMax=xMax+epsilon;
        yMin=yMin-epsilon/2;yMax=epsilon/2+yMax;
        StdDraw.setXscale(xMin,xMax);
        StdDraw.setYscale(yMin,yMax);
    }
